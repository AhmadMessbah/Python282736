...
def edit_c(code,name,brand,description,price,rent_condiotion,rent_price):
    try:
        if valid_code(code) >=1 and  valid_name(name) and valid_name(brand) and valid_name(description) and valid_price(price) and valid_name(rent_condiotion)  and valid_price(rent_price) > 1000:
            True,edit()
        else:
            False,"invalid data"
    except Exception as e:
