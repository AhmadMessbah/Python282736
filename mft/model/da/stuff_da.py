...
class StuffDa:
    ...
    def save(self, stuff):
        self.execute(
            ...
            [stuff.name, stuff.brand, stuff.description, stuff.price, stuff.rent_condiotion, stuff.rent_price],
            commit=True)

    def edit(self, stuff):
        self.execute(
            'update stuff_tbl set name = %s, brand =%s ,description = %s, price =%s,rent_condition=%s ,rent_price=%s where code =%s',
            [stuff.code, stuff.name, stuff.brand, stuff.description, stuff.price, stuff.rent_condiotion,
             stuff.rent_price],
            commit=True)
    ...
    def find_all(self):
        return self.execute("select * from stuff_tbl where deleted =0")
